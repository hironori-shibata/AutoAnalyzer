...
Task3（Perplexity情報収集）とTask3a（定量データ）を統合してレポートを作成する。
...
def create_agent3() -> Agent:
    ...
    return Agent(
        ...
        backstory=(
            ...
            "3. 「{企業名} AI 戦略」「{企業名} ニュース 最新」で注目テーマを収集\n"
...
def create_agent3b() -> Agent:
    ...
    return Agent(
        ...
        goal=(
            "Task3（Perplexity情報収集）とTask3a（競合定量データ）の両情報を統合し、"
        ...
        backstory=(
            ...
            "Task3の定性情報（最新ニュース・業界トレンド・競争優位性の予備分析）と\n"
            "Task3aの定量データ（PER・PBR・時価総額等）を組み合わせ、\n"
            "具体的なエビデンスに基づいた分析を行います。\n\n"
            "【重要ルール】\n"
            "- Task3で収集した注目テーマを必ず反映させること\n"
            "- Task3aのPER/PBR数値はそのまま転記し、数値計算は行わないこと\n"
