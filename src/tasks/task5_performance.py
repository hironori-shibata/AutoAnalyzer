...
def create_task5(ticker: str, edinet_code: str) -> Task:
    ...
    return Task(
        description=(
            ...
            "【手順1: 財務テーブル取得】\n"
            f"IRBankFinancialTableTool（edinet_code='{edinet_code}'）を呼び出して構造化JSONを取得する。\n"
            "このJSONには pl・bs・cf・dividend の4セクションがあり、monetary値はすべて円（1円単位）、"
            "比率・マージンは % で格納されている。\n\n"
            "【手順2: 全指標のトレンド一括計算】\n"
            "手順1で取得したJSONをそのまま IRBankTrendBatchTool の financial_json パラメータに渡し、"
            "売上・利益・ROE/ROA・CF・配当等の全指標のCAGR・トレンドを一括計算すること。\n"
            "⚠️ 財務テーブル指標に対して TrendAnalysisTool を個別に呼ぶことは禁止。"
            "IRBankTrendBatchTool の1回呼び出しで代替できる。\n\n"
            "⚠️ 【単位変換の厳守 – 後続エージェントのDCFに影響する】\n"
            "ツールが返す金額はすべて1円単位の生の数値。\n"
            ...
            "主要セグメント別の過去3〜5年間の売上高推移を取得すること。\n"
            "各セグメントの売上推移は TrendAnalysisTool に渡してセグメント別CAGRを計算する。\n"
