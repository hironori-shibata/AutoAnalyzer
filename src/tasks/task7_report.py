...
def create_task7(
    ...
    return Task(
        description=(
            ...
            "        ⚠️ 有利子負債が0（無借金企業）の場合も interest_bearing_debt=0 を明示的に渡すこと（省略は厳禁）。\n"
            "        ⚠️ target_market_cap（上記5.で取得済み）と必ずセットで渡すこと。\n"
            "        ツールが D/(D+E)=有利子負債/(有利子負債+時価総額) で市場価値ベース負債比率を自動計算する。\n"
            "        無借金企業の場合は D/(D+E)=0% として正しく算出される。\n"
            "        target_market_capが未提供の場合のみデフォルト30%が使われ精度が大幅に低下する。\n"
            ...
            "   - target_ebitda: Agent4のFinancialCalcTool出力のebitda値（円単位）を使用すること。\n"
            ...
            "       - 資本政策イベント（★自社株買い・増配・増資等の直近発表）: 金額・規模を必ず引用すること\n"
            ...
            "⚠️ Agent2（Gemini）のリサーチ結果を必ず参照し、\n"
            "   リスク・課題（セクション9）とカタリスト分析（セクション12）に最新ニュース情報を反映すること。\n"
            "   特にAgent2が収集した資本政策イベント（自社株買い・増配等）は最新ニュースセクション（セクション11）に必ず記載すること。\n"
        ...
        agent=agent,
        context=[task1, task2, task3, task3a, task3b, task4, task5],  # 全Agentの結果を参照
    )
