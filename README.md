...
- **11段階のAIパイプライン**: 有報・競合・決算短信・業績・ニュース・DCF・ショートレポート・最終投資判断まで網羅
...
```
Slack受信 → [11タスク 逐次実行] → Slackレポート送信
               │
               ├─ Task1:   有報リサーチャー              (DeepSeek v3)
               ├─ Task2:   対象企業ディープリサーチ       (Gemini Flash + Google検索)
               ├─ Task3:   競合・業界調査                 (Perplexity Sonar)
               ├─ Task3a:  競合財務データ収集             (DeepSeek v3)
               ├─ Task3b:  競合比較レポート生成           (DeepSeek R1)
               ├─ Task4:   決算短信解析                   (DeepSeek v3)
               ├─ Task5:   業績トレンド分析               (DeepSeek v3)
               ├─ Task6:   業界ニュース・地政学           (Perplexity Sonar)
               ├─ Task7:   統括レポート生成 (DCF)         (DeepSeek v3)  ← Task1〜Task6全参照
               ├─ Task8:   ショートレポート               (ChatGPT via OpenRouter)
               └─ Task9:   最終投資判断                   (DeepSeek R1)
```
...
| Agent2 ディープリサーチャー | Google検索でIR情報・技術動向・市場評価・注目テーマを収集 | Gemini Flash | Google Search Grounding |
| Agent3 競合リサーチャー | 競合企業・業界構造の調査 | Perplexity Sonar | Webリアルタイム検索 |
| Agent3a 競合データ収集 | 競合企業の財務データを数値で収集 | DeepSeek v3 | IR Bank, kabutan |
| Agent3b 競合レポート生成 | 収集データから競合比較レポートを生成 | DeepSeek R1 | Task3/Task3a出力 |
| Agent4 決算短信リサーチャー | 最新決算短信PDF取得・財務指標計算 | DeepSeek v3 | IR Bank PDF, docling |
| Agent5 業績リサーチャー | 複数年の業績トレンド・成長率分析 | DeepSeek v3 | IR Bank 時系列データ |
| Agent6 ニュースリサーチャー | 企業固有ニュース・業界動向・地政学リスク収集 | Perplexity Sonar | Webリアルタイム検索 |
| Agent7 統括マネージャー | 全情報集約、DCF・マルチプル法で企業価値算定、買いレポート生成 | DeepSeek v3 | Task1〜Task6全出力 |
| Agent8 ショートアナリスト | Agent7の前提を破壊し、崩壊ポイントを特定するベアケース分析 | ChatGPT (OpenRouter) | Task7出力 |
| Agent9 最終投資意思決定者 | ブル・ベア両レポートを第三者精査し、確率加重で最終判断を下す | DeepSeek R1 | Task7+Task8出力 |
...
| `OPENROUTER_API_KEY` | Perplexity Sonar / ChatGPT / DeepSeek R1 用 | [OpenRouter](https://openrouter.ai/) |
